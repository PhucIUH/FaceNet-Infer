...
cap = cv2.VideoCapture(
    r"rtsp://admin:space123@192.168.1.47:554/cam/realmonitor?channel=1&subtype=1")
    ...
    if isSuccess:
        ...
        if boxes is not None:
            for box in boxes:
                ...
                #     for points in usr:
                #         frame = cv2.circle(frame, (int(points[0]), int(
                #             points[1])), radius=0, color=(0, 0, 255), thickness=10)
    new_frame_time = time.time()
    fps = 20/(new_frame_time-prev_frame_time)
    prev_frame_time = new_frame_time
    fps = str(int(fps))
    cv2.putText(frame, fps, (20, 70), cv2.FONT_HERSHEY_DUPLEX,
